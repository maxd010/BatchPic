...
ipcMain.handle(
  "process-images",
  async (event, files: ImageFile[], params: ProcessingParams) => {
    try {
      // Create output directory
      const outputRoot = await outputManager.createOutputDirectory(
        files.map((f) => f.path),
      );
...
ipcMain.handle(
  "save-template",
  async (_event, name: string, params: ProcessingParams) => {
    try {
...
ipcMain.handle(
  "estimate-file-size",
  async (_event, filePath: string, params: ProcessingParams) => {
    try {
...
ipcMain.handle(
  "create-output-directory",
  async (_event, inputPaths: string[]) => {
    try {
